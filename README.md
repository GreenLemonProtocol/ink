...
If you want to anonymize one of your transactions, you have to deposit a small amount of DOT on the Green Lemon Protocol contract (ex.: 1 DOT). After a little while, you can withdraw this 1 DOT with a different account. The trick is that nobody can create a link between the depositor account and the withdrawal account. If hundreds of accounts deposit 1 DOT on one side and the other hundreds of accounts withdraw 1 DOT on the other side, then nobody will be able to follow the path where the money moves. Most importantly, we can trigger a third-party contract when we withdraw the DOT. If you pay a fee to the relayer service, that fee can cover the cost of calling a third-party contract. Then the whole logic makes sense.

The technical challenge is that smart contract transactions are also public like any other transaction on the Polkadot network. This is the point where ZKP will be relevant.

When you deposit 1 DOT on the contract, you have to provide a “commitment”. This commitment is stored by the smart contract. When you withdraw 1 DOT on the other side, you have to provide a “nullifier” and a zero-knowledge proof. The nullifier is a unique ID that is in connection with the commitment and the ZKP proves the connection, but nobody knows which nullifier is assigned to which commitment (except the owner of the depositor/withdrawal account).The nullifiers are tracked by the smart contract, so we can withdraw only one deposited DOT with one nullifier.

Commitments are stored in the contract in the form of the Merkle tree, because store every commitment in mapping is very expensive. If somebody wants to call the method she has to give a Merkle proof (in this case it is a list of 10 hashes) that can be easily validated by the smart contract.
